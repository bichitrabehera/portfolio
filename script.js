...
document.addEventListener("DOMContentLoaded", function () {
    ...
    gsap.utils.toArray(".titles").forEach((title, index) => {
        animateSection(title, { opacity: 0, y: 50, duration: 1, delay: index * 0.1 });
    });
    ...
    animateSection(".aboutcont", { opacity: 0, x: -50, duration: 1, stagger: 0.3 });
    animateSection(".skills-vert", { opacity: 0, x: 50, duration: 1, stagger: 0.3 });
    gsap.utils.toArray(".rowskill h2").forEach((skill, index) => {
        animateSection(skill, { opacity: 0, y: 30, duration: 1, delay: index * 0.1 });
    });
    gsap.utils.toArray(".project").forEach((project, index) => {
        animateSection(project, { opacity: 0, y: 50, duration: 1, delay: index * 0.2 });
    });
    animateSection(".contact-section", { opacity: 0, y: 50, duration: 1 });
    gsap.utils.toArray("#contactForm input, #contactForm textarea, .contact-btn").forEach((field, index) => {
        animateSection(field, { opacity: 0, y: 30, duration: 1, delay: index * 0.1 });
    });
